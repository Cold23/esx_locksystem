...
MySQL.ready(function ()
    MySQL.Async.fetchAll("SELECT `plate`, `owner` FROM owned_vehicles",{}, function(data)
        for _,v in pairs(data) do
            local plate = string.lower(ESX.Math.Trim(v.plate))
            owners[plate] = v.owner
        end
    end)
end)

RegisterServerEvent("ls:addHotwire")
AddEventHandler("ls:addHotwire", function(plate)
    hotwired[plate] = 1
end)

ESX.RegisterServerCallback('ls:getHotwires', function(source, cb, plate)
    cb(hotwired[plate])
end)
...
AddEventHandler("ls:retrieveVehiclesOnconnect", function()
    ...
    local data = MySQL.Sync.fetchAll("SELECT `plate`, `owner` FROM owned_vehicles WHERE owner = @owner",{
        ['@owner']  = srcIdentifier,
    })
   for _,v in pairs(data) do
    ...
    for plate, plyIdentifier in pairs(owners) do
        if(plyIdentifier == srcIdentifier)then
            local _plate = string.lower(plate)
